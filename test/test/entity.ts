...
async function select() {
    ...
    await Shop.find(1);
    //2、查询单个实体
    // await Shop.findOne({ shopId: 1 });
    // await Shop.findOne({ shopName: { rel: 'like', value: 'frank' } });
    //3、查询多个实体
    // await Shop.findMany({ shopName: { rel: 'like', value: 'frank' } });
    // 4、查询实体个数
    // await Shop.getCount({ shopName: { rel: 'like', value: 'frank%' } })
...
async function del() {
    ...
    // await Shop.delete(1);
    //3、通过类删除，根据条件
    await Shop.deleteMany({ shopName: { rel: 'like', value: '100' } })
}
